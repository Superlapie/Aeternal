...
public class PlayerUpdating {
	...
	private static void updateAppearance(Player player, PacketBuilder out, Player target) {
		...
		if (skillAnim > 0) {
			...
			properties.putShort(wep.getStandTurnAnim());

			properties.putShort(wep.getWalkAnim());

			properties.putShort(wep.getTurn180Anim());

			properties.putShort(wep.getTurn90CWAnim());

			properties.putShort(wep.getTurn90CCWAnim());
