...
# Check all moon models from moon_cache
moon_models = [39506, 39571, 39572, 39573, 39574, 39575, 39576, 39577, 39578, 39579, 39580, 39581]

for model_id in moon_models:
    model_path = f'_ext/moon_cache/cache/7/{model_id}.dat'
    if os.path.exists(model_path):
        with open(model_path, 'rb') as f:
            raw_data = f.read()
        
        decompressed = unpack_js5_container(raw_data)
        print(f'{model_id}: {len(raw_data)} -> {len(decompressed)} bytes, starts with: {decompressed[:5].hex(" ")}')
    else:
